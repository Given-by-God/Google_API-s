...
class server
{
    ...
    public function parser()
    {
        ...
        for ($i = 0; $i < count($this->post[0]); $i++) {
            $page = file_get_html($this->post[0][$i])->plaintext; // заходим по каждой ссылке

            if (preg_match($patternPhone, $page) || preg_match($patternEmail, $page)) { //смотрим - подходит нам или нет
...
$server = new server(json_decode($_POST['links']));


print_r((json_encode($server->parser())));
?>